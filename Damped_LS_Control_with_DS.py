...
# add Z-axis to data
# x_des = np.vstack((x_des, 0.15*np.ones((1,x_des.shape[1]))))  # (3,611)
x_des = np.vstack((x_des, np.linspace(0.1, 0.18, x_des.shape[1])))  # (3,611)
x_des[0,:] += 0.2
x_des[1,:] += 0.1

# obstacle dimension
obstacle_def = [{"center": np.array([0.24, 0.05, 0.125]), "radius": np.array([0.04, 0.02, 0.04]), "order": np.array([2, 2, 2]), 'eta': 1.05, 'rho': 1.0, "orientation": 0, "color": [0.2, 0.2, 0.2]},
                # {"center": np.array([0.26, 0.18, 0.125]), "radius": np.array([0.04, 0.02, 0.02]), "order": np.array([1, 1, 1]), 'eta': 1.05, "orientation": 0, "color": [0.2, 0.2, 0.2]}
            ]
...
# call DMP
dmp = DMP(no_of_DMPs=3, no_of_basis_func=100, K=400.0, alpha=1, dt=dt, T=T, method="DS", obstacles=obstacle_def)
...
X_g = x_des[:,[-1]]  # [[x_des],[y_des],[z_des]]
# X_g[-1,:] = 0.16
X_0 = x_des[:,[0]]  # [[x_0],[y_0],[z_0]]
X_track = X_0

idx=0
robot.plot_start(T, dt, x_des, obstacles=obstacle_def)
...
while True:
    ...
    robot.memory(X_cord, Y_cord, Z_cord, Ex=None)
   
    X_step, dX_step = dmp.step_with_DS_2012(X_g)
